...
function getTyposBuffer(cmd, lines, token) {
  let res = [];
  return new Promise((resolve, reject) => {
...
var Task = class extends import_events.EventEmitter {
  start(cmd, cwd, args, checkIgnored) {
    ...
    rl.on("line", (line) => {
      ...
      let fullpath = import_path.default.join(cwd, parts[0]);
...
async function activate(context) {
  ...
  subscriptions.push(import_coc5.workspace.registerKeymap(["n"], "typos-next", async () => {
    ...
    let range = import_coc5.Range.create(typo.lnum, characterIndex(content, typo.colStart), typo.lnum, characterIndex(content, typo.colStart + typo.word.length));
